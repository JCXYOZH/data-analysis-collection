...
这是一份北京的租房数据，总计7000 多条记录，分为 8 个同样结构的 CSV 数据文件。
...
```python
# 合并数据文件 
dir = r"C:\data-analysis-collection\renting-apartment-beijing"
data_list = []
for i in range(1, 9):
...
# 保存清洗后的数据 sqlite
engine = create_engine('sqlite:///C:/data-analysis-collection/renting-apartment-beijing/renting-apartment-beijing.db')
data.to_sql('renting-apartment-beijing', con=engine, index=False, if_exists='append')
```
...
![listing-configuration](./images/listing-configuration.png)

## 3.5 交通分析

从地理位置上来看，交通越便利，租金也越贵，这个符合一般认知。

![subways-number](./images/subways-number.png)

![subways-distance](./images/subways-distance.png)
